...
(async function enhanceWorkerOperations() {
  ...
  if (!session.email || typeof fetchWorkerEmergencyAlerts !== 'function') return;
  // The daily checklist + report now lives in its own dashboard panel
  // (see pages/worker-dashboard.html). This block only surfaces open alerts.
  ...
  panel.innerHTML = `<div class="worker-danger-mark is-active"><span aria-hidden="true">!</span><div><strong>Emergency attention required</strong><p>Review the alert below and contact the admin team if you cannot complete today's routine.</p></div></div><div class="worker-alert-list"><h3>Emergency alerts</h3>${alerts.map(alert => `<div class="worker-alert-row"><strong>${alert.alert_type.replaceAll('_', ' ')}</strong><span>${alert.message}</span></div>`).join('')}</div>`;
